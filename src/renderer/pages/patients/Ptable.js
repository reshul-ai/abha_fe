...
const Patienttable = () =>{
    ...
    const getPatientsData = () => {
        //console.log("called");
        window.require("electron").ipcRenderer.send("getNotRecoveredPatientData",{});

        window.require("electron").ipcRenderer.on("getNotRecoveredPatientData", (e,data) => {
    ...
    const getMIA = ((patientId) => {
      // const sessions = sessionResultsData?.items?.filter(session => session.patientId === patientId);
      // console.log((sessions));
      // let totalMIA = 0;

      // for (let i = 0; i < sessions.length; i++) {
      //   if (sessions[i].MIA !== undefined) {
      //     totalMIA += sessions[i].MIA;
      //   }
      // }

      // return Math.floor(totalMIA / sessions.length);
            ...
            <TableContainer component={Paper}>
                    <Table sx={{ minWidth: 700 }} aria-label="customized table">
                        ...
                        <TableRow>
                            ...
                            <StyledTableCell align="left">Last session On</StyledTableCell>
