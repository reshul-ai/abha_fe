...
import SessionBci from '../../../session/SessionBci';

import { Button, Modal } from "react-bootstrap";
import { Link } from 'react-router-dom';

const Upcomingsession=({patientData,sessionData})=>{
        ...
        const [upcomingSessionData, setUpcomingSessionData] = useState({items:[]});
        ...
        const getUpcomingSessions = (() => {
                // 
                window.require("electron").ipcRenderer.send("getUpcomingSessions",patientData?.id);

                window.require("electron").ipcRenderer.on("getUpcomingSessions", (e,data) => {
                        data = data.items.filter(item => item.patientId === patientData.id);
                        console.log(data);
                        setUpcomingSessionData({items:data});
                });
        });
        ...
        const handleBciShow = (id) => {
                setCurrSession(upcomingSessionData.items.find((session) => session.id === id));
                setShowBci(true);
        };
        ...
        <div className='col' style={{'paddingLeft':'0%','paddingRight':'0%'}}>
                <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 700 }} aria-label="customized table">
                                ...
                                <TableBody>
                                {upcomingSessionData.items.map((row) => (
                                <StyledTableRow key={row.id}>
                                        {/* <StyledTableCell component="th" scope="row">
                                                {row.name}
                                        </StyledTableCell> */}
                                        <StyledTableCell align="left" onClick={() => handleBciShow(row.id)} style={{ "cursor": "pointer" }}>{row.id}</StyledTableCell>
                                        <StyledTableCell align="left" onClick={() => handleBciShow(row.id)} style={{ "cursor": "pointer" }}>{patientData?.name}</StyledTableCell>
                        ...
                        <div>
                                <Modal size="lg" show={showBci} onHide={handleBciClose}>
                                        ...
                                        <div className='col text-center Assessments-label' style={{"borderRight": "1px solid silver"}}>
                                                ...
                                                <div className='row Assessments-labelValue'><Link to={`/individualpatients/${currSession.PatientId}`}><div className='col text-center'>{patientData?.name}</div></Link></div>
                                        </div>
                                        <div className='col text-center Assessments-label' style={{"borderRight": "1px solid silver"}}>
